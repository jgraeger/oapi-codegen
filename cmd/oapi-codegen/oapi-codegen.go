...
func main() {
	...
	flag.StringVar(&flagGenerate, "generate", "types,client,server,spec",
		`Comma-separated list of code to generate; valid options: "types", "client", "chi-server", "server", "gin", "gorilla", "bunrouter", "spec", "skip-fmt", "skip-prune"`)
...
func generationTargets(cfg *codegen.Configuration, targets []string) error {
	...
	for _, opt := range targets {
		...
		case "bunrouter", "bunrouter-server":
			opts.BunrouterServer = true
		case "strict-server":
			opts.Strict = true
		case "client":
			opts.Client = true
